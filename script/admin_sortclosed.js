  ...
  $("#sortbox").change(function () {
    ...
    if (document.getElementById("sdate").checked) {
      if (document.getElementById("sasc").checked) {
        xmlhttp.open("GET", "admin_sort.php?sortc=0&order=0", true);
        xmlhttp.send();
      } else if (document.getElementById("sdesc").checked) {
        xmlhttp.open("GET", "admin_sort.php?sortc=0&order=1", true);
        xmlhttp.send();
      }
    } else if (document.getElementById("stotal").checked) {
      if (document.getElementById("sasc").checked) {
        xmlhttp.open("GET", "admin_sort.php?sortc=1&order=0", true);
        xmlhttp.send();
      } else if (document.getElementById("sdesc").checked) {
        xmlhttp.open("GET", "admin_sort.php?sortc=1&order=1", true);
        xmlhttp.send();
      }
    }
  });
